...
public class WordleGWindow {
    ...
    public static final Color ALL_CORRECT_COLOR = new Color(35,213,213);
    ...
    // The color used if you messed up
    public static final Color OOPS_COLOR = new Color(204, 0, 0);
 
    /**
     * Creates a new WordleGWindow object and displays it on the screen.
     */

    public WordleGWindow(Wordle wordle) {
        JFrame frame = new JFrame("Wordle");
        frame.setBackground(Color.WHITE);
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setLayout(new BorderLayout());
        canvas = new WordleCanvas(wordle);
...
class WordleCanvas extends JComponent implements KeyListener, MouseListener {
    ...
    public WordleCanvas(Wordle wordle) {
        this.wordle = wordle;
    ...
    public void hint() {
        boolean[] arrayColWithoutGreenLetter = new boolean[N_COLS];
        String[] nameOfPlaces = {"first", "second", "third", "fourth", "fifth"};
        for(int row = 0; row < N_ROWS; row++) {
            for(int col = 0; col < N_COLS; col++) {
                if(grid[row][col].getColor() == CORRECT_COLOR) {
                    arrayColWithoutGreenLetter[col] = true;
                }
            }
        }
        for(int i = arrayColWithoutGreenLetter.length - 1; i>=0; i--) {
            if(arrayColWithoutGreenLetter[i] != true) {
                showMessage("The " + nameOfPlaces[i] + " letter is \"" + wordle.getSecretWord().substring(i,i+1).toUpperCase() + "\"");
            } 
        }
    }
    ...
    public void resetBoard() {
        finished = false;
        wordle.setSecretWord();
        for(int row = 0; row < N_ROWS; row++) {
            for(int col = 0; col< N_COLS; col++) {
                grid[row][col].setColor(UNKNOWN_COLOR);
                grid[row][col].setLetter(" ");
            }
        }
        setCurrentRow(0);
        for(WordleKey key : keys.values()) {
            key.setColor(KEY_COLOR);
        }
        showMessage("Board was reset!");
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (key != null) {
            if (key.equals("Reveal")) {
                this.showMessage("The secret word is " + this.wordle.getSecretWord());
    ...
    private void initWordleKeys() {
        ...
        keys.put("Reset", new WordleKey(15, 200, 65, "Reset"));
        keys.put("Reveal", new WordleKey(15, 270, 65, "Reveal"));
        keys.put("Hint", new WordleKey(20, 340, 50, "Hint"));
    }
